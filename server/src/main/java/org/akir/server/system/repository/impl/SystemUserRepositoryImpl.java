...
import org.akir.common.core.repository.BPWhereBuilder;
...
public class SystemUserRepositoryImpl extends BaseRepositoryImpl<User, Long> implements SystemUserRepository {
     ...
     */
    @Override
    public @Transactional(readOnly = true) Optional<User> findByAuthnUserId(String authnUserId) {
        return Optional.ofNullable(factory.create(em, User.class, "user").where("user.authnUserId").eq(authnUserId).getSingleResult());
    }
}
